...
fn convert_api_key_response<T: serde::de::DeserializeOwned>(
    ...
    let array_value = match &json {
        serde_json::Value::Object(obj) => {
            match obj.get(wrapper_key) {
                ...
                Some(other) => {
                    return Err(format!(
                        "Expected array or null for key '{}', got: {:?}",
                        wrapper_key, other
                    ))
                }
            }
        ...
        _ => {
            return Err(format!(
                "Unexpected response format: expected object with key '{}' or array",
                wrapper_key
            ))
        }
    };
...
fn convert_to_management_format<T: serde::Serialize>(
    data: &T,
...
fn normalize_openai_compatible_providers(
...
#[tauri::command]
pub async fn set_gemini_api_keys(
...
#[tauri::command]
pub async fn add_gemini_api_key(
    state: State<'_, AppState>,
    key: GeminiApiKey,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn set_claude_api_keys(
    state: State<'_, AppState>,
...
#[tauri::command]
pub async fn add_claude_api_key(
    state: State<'_, AppState>,
    key: ClaudeApiKey,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn set_codex_api_keys(
    state: State<'_, AppState>,
...
#[tauri::command]
pub async fn set_vertex_api_keys(
    state: State<'_, AppState>,
    keys: Vec<VertexApiKey>,
) -> Result<(), String> {
    let port = state.config.lock().unwrap().port;
    let url = crate::get_management_url(port, "vertex-api-key");
...
#[tauri::command]
pub async fn add_vertex_api_key(
    state: State<'_, AppState>,
    key: VertexApiKey,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn get_openai_compatible_providers(
    state: State<'_, AppState>,
) -> Result<Vec<OpenAICompatibleProvider>, String> {
    let port = state.config.lock().unwrap().port;
...
#[tauri::command]
pub async fn set_openai_compatible_providers(
    state: State<'_, AppState>,
    providers: Vec<OpenAICompatibleProvider>,
) -> Result<(), String> {
    let port = state.config.lock().unwrap().port;
    let url = crate::get_management_url(port, "openai-compatibility");
    ...
    if !response.status().is_success() {
        ...
        let text = response.text().await.unwrap_or_default();
        return Err(format!(
            "Failed to set OpenAI-compatible providers: {} - {}",
            status, text
        ));
    }
...
#[tauri::command]
pub async fn add_openai_compatible_provider(
    state: State<'_, AppState>,
...
#[tauri::command]
pub async fn delete_openai_compatible_provider(
    state: State<'_, AppState>,
    index: usize,
) -> Result<(), String> {
    let mut providers = get_openai_compatible_providers(state.clone()).await?;
